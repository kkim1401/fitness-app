    ...
    return (
      <div>
        <form onSubmit={this.submitInfo}>
          <h3>User's Info</h3>
          <label>
                    Name: <input type="text" name="name" ref={(node) => { this.name = node; }} defaultValue={name || ''} />
          </label>
          <label>
                    Gender: <input type="text" name="gender" ref={(node) => { this.gender = node; }} defaultValue={gender || ''} />
          </label>
          <label>
                    Age: <input type="number" name="age" ref={(node) => { this.age = node; }} defaultValue={age || ''} />
          </label>
          <h3>1RM</h3>
          <label>
                    Squat: <input type="number" name="squat" ref={(node) => { this.squat = node; }} defaultValue={squat || ''} />
          </label>
          <label>
                    Bench: <input type="number" name="bench" ref={(node) => { this.bench = node; }} defaultValue={bench || ''} />
          </label>
          <label>
                    Deadlift: <input type="number" name="deadlift" ref={(node) => { this.deadlift = node; }} defaultValue={deadlift || ''} />
          </label>
